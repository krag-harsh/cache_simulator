...
def search_dm(addr,tag_f,c1,c2,tag_f2):       #function to search the address using direct mapping technique
    ...
    if(stf==addr[:nb_tf]):              #checking if the given location has the same tag field or not(if yes than cache hit)
        print("CACHE HIT was found in level 1 cache only")
        ...
        print("Enter",nb_cl2,"bits to search in cache level 2 as it was not found in level 1")
        b=input("Enter value here: ")
        lioo=int(b,2)
        stf2=tag_f2[lioo]
        if(stf2==addr[:nb_tf]):
            print("CACHE HIT the data was found in level 2 cache")
            r = int(math.log2(len(c2[0])))  # r stores the number of bit in which cache is stored
            bi = int(addr[-r:], 2)  # slicing the address from the given address and then converting it to decimal form
            print("the value found at the given location is : ", c2[lioo][bi])  # printing the value at the given location
        else:
...
def search_am(addr,tag_f,c1,c2,tag_f2):       #function to search the address using associative mapping technique
    nb_tf = len(tag_f[0])               #finding the length of the bit required to store the tag field
    ...
    for i in range(len(c1)):            #start searching on each line of the cache
        if(se==tag_f[i]):               #checking if the given location has the same tag field or not(if yes than cache hit)
            flag=1
            print("CACHE HIT address was found in level 1 only\t")
            r = int(math.log2(len(c1[0])))          # r stores the number of bit in which cache is stored
            bi = int(addr[-r:], 2)               # slicing the address from the given address and then converting it to decimal form
    ...
    if(flag==0):
        ...
        for i in range(len(c2)):
            if(se==tag_f2[i]):
                flag2=1
                print ("CACHE HIT the address was found in level 2 cache")
                r = int(math.log2(len(c1[0])))  # r stores the number of bit in which cache is stored
                bi = int(addr[-r:],2)  # slicing the address from the given address and then converting it to decimal form
                print("the value found at the given location is : ", c2[i][bi])  # printing the value at the given location
                break
        if(flag2==0):
            print("cache miss,address not found either in cache level 1 or 2")
...
def search_nam(n,addr,tag_f,c1,c2,tag_f2):            #function to search the address using n was associative mapping technique
    no_sets=int(cl/n)
    ...
    for i in range(n*setno,n*setno+n):
        if(se==tag_f[i]):                       #checking if the given location has the same tag field or not(if yes than cache hit)
            flag = 1
            print("CACHE HIT address was found in level 1 cache")
            r = int(math.log2(len(c1[0])))           # r stores the number of bit in which cache is stored
            bi = int(addr[-r:], 2)                  # slicing the address from the given address and then converting it to decimal form
            print("the value found at the given location is : ", c1[i][bi])         #printing the value at the given location
            break
    if (flag == 0):
        flag2=0
        no_sets2 = no_sets*2
        noofbit_for_sets2 = int(math.log2(no_sets2))  # finding the length of the bit required to sets
        print("address not found in cache level 1 going to search in level 2")
        print("Enter", noofbit_for_sets2, "bits of the set number to search in cache level 2 as it was not found in level 1")
        setno2 = input("Enter value here: ")
        setno2=int(setno2,2)
        for i in range(n * setno2, n * setno2 + n):
            if (se == tag_f2[i]):  # checking if the given location has the same tag field or not(if yes than cache hit)
                flag2 = 1
                print("CACHE HIT location was found in level 2 cache")
                r = int(math.log2(len(c1[0])))  # r stores the number of bit in which cache is stored
                bi = int(addr[-r:],2)  # slicing the address from the given address and then converting it to decimal form
                print("the value found at the given location is : ", c2[i][bi])  # printing the value at the given location
                break
...
N=128      #predifining total size of main memory
...
if(majchoice==2):
    cl = int(input("enter numbers of cache lines for the smaller cache :\t"))
...
if(not pow2c(s*N)):                 #checking if the given values are of power 2 or not
    ...
    for i in range(cl2):
        row = []
        for j in range(b):
            row.append(i+j)             #putting some values in the second cache before our code start
        cac2.append(row)
    ...
    if(o==1):
        ...
        for i in range(cl):
            ...
            tf.append(row)
        for i in range(cl2):
            row=("{0:0%db}" % nb_tf).format(i)          #initialsing tag values 2 with some data each in binary format of length find earlier
            tf2.append(row)
        ...
        while(wanttocont==1):
            ...
            if(choi==1):
                ...
                search_dm(s, tf, cac,cac2,tf2)                   #calling the function for searching the given address
        ...
        for i in range(cl2):
            row = ("{0:0%db}" % nb_tf).format(
                i)  # initialsing tag values 2 with some data each in binary format of length find earlier
            tf2.append(row)
        ...
        while (wanttocont == 1):
            ...
            if (choi == 1):
                ...
                s = input("Enter the address to be fetched in binary format :  ")
                search_am(s, tf, cac,cac2,tf2)                                       #calling the function for searching the given address
                wanttocont = int(input("Enter 1 if you want to continue : "))
            elif (choi == 2):
                print("Enter", b, "values you want to store in cache")
                sentdata = []
        ...
        if(not pow2c(n)):
            ...
            while (wanttocont == 1):
                ...
                if (choi == 1):
                    ...
                    s = input("Enter the address to be fetched in binary format : ")
                    search_nam(n, s, tf, cac,cac2,tf2)                           #calling the function for searching the given address
                    wanttocont = int(input("Enter 1 if you want to continue : "))
                elif (choi == 2):
